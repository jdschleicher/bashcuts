...
If the cache is older than 6 hours, `az-Get-AzDevOpsAssigned`, `az-Show-Assigned`, `az-Get-AzDevOpsMentions`, `az-Show-Tree`, `az-Show-Board`, `az-Show-Epics`, `az-Show-Features`, `az-Show-Orphans`, `az-Show-CurrentSprint`, `az-Show-ItemsBySprint`, `az-Show-Areas`, `az-Show-Iterations`, and `az-Find-AzDevOpsWorkItem` each print a one-line `WARNING stale (last sync: ...)` notice above their output and still render the cached data.
