...
**Publish** — `az-Publish-AzDevOpsDraft` resolves the iteration/area once (picker or `-Iteration`/`-Area`), then walks the tree **parents-first**, creating each item via the same `Invoke-AzDevOpsWorkItemCreate` + `Invoke-AzDevOpsParentLink` path the single-shot creators use, and adds the parent link in the same pass. A live progress bar reports how far the publish has landed:
...
The dashboard has two modes, switched with the **Agenda / Create** tabs below the header. **Agenda** is the read view described here. **Create** is the Azure DevOps creation surface, itself split into sub-tabs — **Work item**, **Draft**, **Timer**, and **Unplanned**. **Work item** files a **User Story, Task, Feature, Epic, or a Feature with child Stories** from the browser: the form is a function of the type you pick (only the fields that type carries render), area / iteration / parent choices come from the local cache, and a submit runs the same non-interactive `az boards work-item create` core the terminal `az-New-*` commands wrap — so the browser and the terminal produce identical work items. **Draft** is the brain-dump builder (see below). **Timer** runs a focus session in the browser and posts a debrief comment (see below). **Unplanned** captures a firefight's interruptions and files them against your daily unplanned-work story (see below). The active mode is reflected in the URL, so `#create` deep-links straight to it and the back button returns you to Agenda. Create mode checks it can reach the backend on first open via `POST /api/create/ping`; opened without the local server, it reports "offline — sample mode."
...
The **Timer** sub-tab is the browser counterpart of the terminal `Start-TimerSession` (see [Timer sessions](#timer-sessions)) without the WPF / console stopwatch. Pick a registered integration (the built-in **Azure DevOps** one is preselected when it's the only one) and one of its cached assigned items, set the minutes, and **Start timer** — the countdown runs in the browser (a plain JS interval with a progress bar that sits under `prefers-reduced-motion`; the clock is a `role="timer"` region that doesn't announce every tick). **+5 min** extends it, **End & debrief** stops early, **Cancel** drops back to setup. When the countdown ends (or you end it early) a debrief form collects your **Debrief** and **Next** notes — and, when the integration can resolve items, a **Work complete — resolve this item** checkbox. **Post debrief** sends it to `POST /api/timer/post`, which composes the comment with the same `Format-TimerCommentBody` helper the terminal uses and posts it via the integration's `AddComment` (then `CloseItem` if you ticked resolve) — so a browser debrief and a terminal debrief land as the identical comment. A successful post offers the same three-way as the terminal: **Same item** (a fresh session on the same item), **Pick another**, or **Done**. Every item title reaches the page as text (never markup), the post endpoint is `POST` + `application/json` on `127.0.0.1` only, and your `az` token never leaves the server.

The **Unplanned** sub-tab is the browser counterpart of the terminal `az-Start-UnplannedWork` (see [Unplanned work sessions](#unplanned-work)) without the WPF / console stopwatch — the browser is headless, so you report the **minutes spent** rather than run a live clock. Give the firefight a **title**, log its **interruptions** as you go (each row is timestamped when you add it), write a **Debrief** and an optional **Future opportunity**, and **File firefight**. That single `POST /api/unplanned/firefight` reuses the exact terminal helpers: it resolves-or-creates today's **Unplanned Work — yyyy-MM-dd** story (`Get-UnplannedWorkDailyStory`, with the cached-id reuse via `Get-UnplannedCachedStoryId` so a repeat firefight grabs the same story), creates and links the child **Task** (`New-UnplannedWorkTask`), flushes the captured items to the Task description (`Save-UnplannedItemsToTask` → `Format-UnplannedItemsDescription`), posts the debrief comment (`Format-UnplannedDebriefComment`), and records the session in the day's ledger (`Add-UnplannedLedgerEntry`) — so a browser firefight and a terminal one land identically. For the day's **first** firefight you pick a **parent Feature** for the new daily story from a cache-driven list (the same `hierarchy.json` source `Read-UnplannedParentFeature` reads — a real pick, not free text); once the story exists that picker drops away. At end of day, **Post daily roll-up** (`POST /api/unplanned/rollup`) totals every filed firefight and comments the roll-up on the daily story (`Format-UnplannedDailyDebrief`). Every title reaches the page as text (never markup), both write endpoints are `POST` + `application/json` on `127.0.0.1` only, and your `az` token never leaves the server. `$env:AZ_AREA` / `$env:AZ_ITERATION` must be set (run `az-Connect-AzDevOps`) for the browser capture to file.
...
- `GET /api/timer/options` — cache-backed picker data for the **Timer** sub-tab: the registered timer integrations (`$script:TimerIntegrations`), each with its `name` / `description` / `supportsMentions` / `canResolve` and its pickable items from that integration's `FetchItems` (the Azure DevOps one reads the assigned-items cache). Read-only, no live `az`, so it stays a plain GET.
- `POST /api/timer/post` — composes and posts a session debrief, reusing `Format-TimerCommentBody` + the picked integration's `AddComment` (and `CloseItem` when `resolve` is set), the same helpers `Start-TimerSession` calls. Body `{ integration, id, totalSeconds, elapsedSeconds, interrupted, debrief, next, resolve? }`. Returns `{ "ok": true, id, resolved }` (with `resolveError` if only the resolve step failed), or `{ "ok": false, "error": … }` on an auth / `az` failure so the form can surface it.
- `GET /api/unplanned/options` — cache-backed picker data for the **Unplanned** sub-tab: today's daily-story title, its cached id (`Get-UnplannedCachedStoryId`, `0` when the story isn't created yet), and the parent-Feature candidates from `hierarchy.json` (the same source `Read-UnplannedParentFeature` picks from). Read-only, no live `az`, so it stays a plain GET.
- `POST /api/unplanned/firefight` — files one captured firefight, reusing the terminal helpers end-to-end: `Get-UnplannedWorkDailyStory` (find-or-create today's story, headless — the chosen parent Feature + `$env:AZ_AREA` / `$env:AZ_ITERATION` are passed so nothing prompts), `New-UnplannedWorkTask`, `Save-UnplannedItemsToTask` (→ `Format-UnplannedItemsDescription`), `Format-UnplannedDebriefComment` + `Add-AzDevOpsDiscussionComment`, and `Add-UnplannedLedgerEntry`. Body `{ title, minutes, parentFeatureId?, items:[{ time, text }], debrief?, future? }`. Returns `{ "ok": true, storyId, taskId, minutes, itemCount, posted }` (with `postError` if only the comment step failed), or `{ "ok": false, "error": … }` on a missing classification / auth / `az` failure.
- `POST /api/unplanned/rollup` — posts the end-of-day roll-up on today's daily story, reusing `Format-UnplannedDailyDebrief` over the day's ledger (the same read `New-UnplannedWorkDebrief` does) + `Add-AzDevOpsDiscussionComment`. No body params. Returns `{ "ok": true, storyId, count, totalMinutes }`, or `{ "ok": false, "error": … }` when the ledger is empty or the post fails.

Each tile is populated from a real source, reusing the same WIQL defaults and Outlook module the rest of the toolkit uses:

- **Today's Agenda** — today's calendar events from `ol-Get-OutlookAgenda` (desktop Outlook), with the Teams join link when the meeting carries one. A meeting that carries an agenda body or invitees expands in place (click the row, or tab to it and press Enter) to reveal the full meeting body and the attendee list with each person's RSVP (accepted / declined / tentative / no response); the body has a single source, so the clamped preview can never drift from the expanded text. A bare meeting with neither body nor attendees renders as a plain row, exactly as before.
- **Events to Prepare For** — the next two weeks of meetings, each row carrying a date, the meeting's start time and location (a Teams join link when the meeting has one, the room otherwise — the same agenda-style detail the Today's Agenda tile shows), the same click-to-expand agenda body + attendee-RSVP detail, and a marker you can toggle between "prep still needed" and "all set." That choice is saved server-side by the meeting's event id (a small `prep-markers.json` beside the tile cache), so a meeting you mark "all set" stays that way when the cache reloads.
